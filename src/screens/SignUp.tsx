      ...
      <VStack flex={1} px={16} py={40}>
        <Center flex={1}>
          ...
          <VStack mt={48} w="$full">
            ...
            <VStack gap={8}>
              <Text fontFamily="$body" fontSize="$md" fontWeight="$medium" color="$textDark800">CPF</Text>
              <Input placeholder="Digite seu CPF" keyboardType="numbers-and-punctuation" />
            </VStack>

            <VStack gap={8}>
              <Text fontFamily="$body" fontSize="$md" fontWeight="$medium" color="$textDark800">Data de nascimento</Text>
              <Input placeholder="Digite sua data de nascimento" keyboardType="numeric" />
            </VStack>

            <VStack gap={8}>
              <Text fontFamily="$body" fontSize="$md" fontWeight="$medium" color="$textDark800">Senha</Text>
              <Input placeholder="Digite sua senha" type="password" />
            </VStack>

            <VStack gap={8}>
              <Text fontFamily="$body" fontSize="$md" fontWeight="$medium" color="$textDark800">Confirme sua senha</Text>
              <Input placeholder="Digite sua senha novamente" type="password" />
            </VStack>

            <Button title="Entrar" onPress={() => { }} />

            <Text fontFamily="$body" fontSize="$md" fontWeight="$regular" color="$textDark800" textAlign="center" mt={30}>Já tem uma conta? <Text color="$teal600" onPress={() => { }}>Entre agora</Text></Text>
          </VStack>
        </Center>
      </VStack>
    </ScrollView>
